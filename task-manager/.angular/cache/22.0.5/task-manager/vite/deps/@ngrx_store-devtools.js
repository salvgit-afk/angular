...
} from "./chunk-35A43TGR.js";
import {
  toSignal
} from "./chunk-E4QV26RV.js";
import {
  ErrorHandler,
  InjectionToken,
  NgZone,
...
function isActionCreatorPayload(action) {
  return typeof action === "object" && action !== null && !("type" in action) && typeof action.selected === "number" && Array.isArray(action.args);
}
function getActionCreatorArgs(actionCreator) {
  ...
  return parenthesizedArgs[1].split(",").map((arg) => arg.replace(/^\s*\.{3}/, "").split("=")[0].trim()).filter((arg) => arg !== "");
}
function getActionCreatorDescriptors(actionCreators) {
...
var evalArg = (arg) => arg === "" ? void 0 : (0, eval)(`(${arg})`);
var DevtoolsExtension = class _DevtoolsExtension {
  constructor(devtoolsExtension, config, dispatcher) {
    ...
    this.actionCreatorDescriptors = config.actionCreators ? getActionCreatorDescriptors(config.actionCreators) : void 0;
